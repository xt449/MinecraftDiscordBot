 ...
 * All Rights Reserved
 */
class DiscordBanCommand extends Command {

	DiscordBanCommand() {
		this.name = "ban";
		this.aliases =  new String[0];
		this.help = "Use this command to ban a user from the Discord server.";
		this.arguments = " <@User>";
	...
	protected void execute(CommandEvent commandEvent) {
		if(commandEvent.getArgs().length() > 0) {
			...
			if(members.size() > 0) {
				MinecraftDiscordBot.discordBot.guild.getController().ban(members.get(0), 0).queue();
			} else {
